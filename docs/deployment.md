...
Use `litgraph-gateway` when clients already speak the OpenAI Chat Completions
protocol and need centralized virtual keys, model-group permissions, request
limits, daily budgets, weighted routing, circuit-breaker failover, and SSE
relay.

```bash
cargo run -p litgraph-gateway -- keygen --id local --group local-chat
cargo run -p litgraph-gateway -- serve --config gateway.toml --bind 0.0.0.0:8080
```

An Ollama deployment uses `provider = "ollama"` and a base URL such as
`http://127.0.0.1:11434/v1`; it does not need an upstream API key. Authenticated
OpenAI-compatible deployments use `provider = "openai"` plus `api_key_env`.
Clients point their OpenAI SDK base URL at the gateway `/v1` path and use the
plaintext virtual key generated by `keygen`. Only its Argon2id hash is stored.

Gateway v1 keeps spend state in memory and loads keys from configuration. Plan
restart semantics accordingly; persistent spend, key-management APIs,
embeddings proxying, and latency-aware routing are explicit non-goals for v1.

## Package Python applications

Install the published package with `pip install litgraph`. Published wheels use
PyO3’s stable ABI, allowing one platform wheel to cover CPython 3.9–3.13+. For
source development:
