...
| Vector-indexed semantic search on Store | Memory recall by meaning | ✅ `litgraph_core::SemanticStore` — wraps any `Store` with an `Embeddings` provider; `put(ns, key, text, value)` embeds + stores, `semantic_search(ns, query, k)` ranks Rayon-parallel cosine. Python: `litgraph.store.SemanticStore(store, embedder)`. (iter 185) Plus `bulk_put(ns, items, ttl_ms, chunk_size, concurrency)` (iter 222) — single-call bulk indexer that embeds via iter 183's chunked Tokio fan-out. Plus `bulk_delete(ns, keys)` (iter 223) — pair to bulk_put for retention sweeps / namespace cleanup. Closes the LangGraph `BaseStore::mset` + `mdelete` parity gap. |
