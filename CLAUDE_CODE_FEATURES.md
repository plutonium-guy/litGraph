...
| Eval statistical significance | Is the change real or noise? | ✅ `litgraph_core::mcnemar_test(&baseline, &current) -> Vec<McNemarResult>` (iter 290) — McNemar's chi-squared test for paired binary outcomes. Per-scorer 2×2 contingency table `{a: pass-pass, b: pass-fail (regression), c: fail-pass (improvement), d: fail-fail}`. Continuity-corrected statistic `(|b-c|-1)²/(b+c)`. Two-tailed p-value via the χ²(1) ↔ Z² identity + Abramowitz & Stegun erf approximation (max error ~1.5e-7). `significant_at_05` boolean for CI gates. `small_sample` flag when `b+c < 25` — the chi-squared approximation is unreliable in that regime; users are advised to prefer an exact binomial test (out-of-scope here, flagged so callers know). Pairs with iter-289 `detect_drift`: drift surfaces the cases that moved; mcnemar_test answers "is the movement statistically meaningful?". |
