...
- ✅ Built-in tools (~23): Calculator, HttpRequest, ReadFile/WriteFile/ListDirectory, Shell, SqliteQuery, BraveSearch/Tavily/DuckDuckGo, TavilyExtract (iter 141), Mcp, Whisper, Dalle, Tts, CachedTool, PythonRepl, Webhook, GmailSend (iter 148), WebFetch (iter 151), **TimeoutTool + RetryTool** (iter 159, tool resilience wrappers), **CurrentTimeTool** (iter 279, date-aware reasoning), **RegexExtractTool** (iter 280, structured-data extraction from unstructured text)
